...
export default function CreateFeedsChannelModal({ isOpen, onClose, workspaceId }) {
  ...
  const validateForm = () => {
    ...
    feedRows.forEach((row, index) => {
      ...
      if (!row.feedUrl.trim()) {
        newErrors[`feedRows.${index}.feedUrl`] = "URL, topic, or keyword is required"
      }
    })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("Creating-workspace-Feeds-Channel", {
        body: {
          name: channelName,
          feeds: feedRows,
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-6">
                  <div>
                    <label htmlFor="channelName" className="block text-sm font-medium text-design-primaryGrey mb-1">
                      Channel Name
                    </label>
                    <input
                      type="text"
                      ...
                      className={`input w-full px-3 py-2 border ${
                        errors.channelName ? "border-semantic-error" : "border-design-greyOutlines"
                      } rounded-lg focus:outline-none focus:ring-2 focus:ring-button-primary-cta`}
                      placeholder="Enter channel name"
                      disabled={isSubmitting}
                    />
                    {errors.channelName && (
                      <p className="mt-1 text-xs text-semantic-error">{errors.channelName}</p>
                    )}
                  </div>
                  
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <h4 className="text-sm font-medium text-design-black">Feed Sources</h4>
                    </div>
                      ...
                      >
                        <div>
                          <input
                            type="text"
                            value={row.feedName}
                            onChange={(e) => handleRowChange(index, "feedName", e.target.value)}
                            className={`input w-full px-3 py-2 border ${
                              errors[`feedRows.${index}.feedName`] ? "border-semantic-error" : "border-design-greyOutlines"
                            } rounded-lg focus:outline-none focus:ring-2 focus:ring-button-primary-cta`}
                            placeholder="Feed Name"
                            disabled={isSubmitting}
                          />
                          {errors[`feedRows.${index}.feedName`] && (
                            <p className="mt-1 text-xs text-semantic-error">{errors[`feedRows.${index}.feedName`]}</p>
                          )}
                        </div>
                        <div className="relative">
                          <input
                            type="text"
                            value={row.feedUrl}
                            onChange={(e) => handleRowChange(index, "feedUrl", e.target.value)}
                            className={`input w-full px-3 py-2 border ${
                              errors[`feedRows.${index}.feedUrl`] ? "border-semantic-error" : "border-design-greyOutlines"
                            } rounded-lg focus:outline-none focus:ring-2 focus:ring-button-primary-cta`}
                            placeholder="Enter URL, Topic or Keyword"
                            disabled={isSubmitting}
                          />
                          {errors[`feedRows.${index}.feedUrl`] && (
                            <p className="mt-1 text-xs text-semantic-error">{errors[`feedRows.${index}.feedUrl`]}</p>
                          )}
